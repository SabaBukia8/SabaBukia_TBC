...
class UserCollectionsRepositoryImpl @Inject constructor(
    ...
    override suspend fun getAllCollections(): Flow<Resource<List<Collection>>> = executeNetworkOperationWithFallback(
        networkOperation = {
            flow {
        ...
        fallbackOperation = {
            flow {
                ...
                try {
                    collectionDao.getAllCollections(userId).collect { entities ->
                        val collections = entities.map { entity ->
                            try {
                                ...
                                val totalCards = collectionCardDao.getTotalCardCount(entity.id, userId)
                                val totalValue = collectionCardDao.getTotalValue(entity.id, userId)
                                entity.toDomain(totalCards, totalValue)
                            } catch (e: Exception) {
                    ...
                    emit(Resource.Error(cacheError.message ?: "Failed to load collections from cache"))
                    emit(Resource.Loading(false))
                }
            }
        }
    ...
    override suspend fun getCollectionById(collectionId: Long): Flow<Resource<Collection?>> = executeNetworkOperationWithFallback(
        networkOperation = {
            flow {
                emit(Resource.Loading(true))
                try {
                    ...
                    val firestoreCollection = firestoreDataSource.getCollectionById(userId, firestoreId)
                    ...
                    if (firestoreCollection != null) {
                        ...
                        val cards = firestoreDataSource.getCardsOnce(userId, firestoreCollection.id)
        ...
        fallbackOperation = {
            flow {
                ...
                try {
                    val localEntity = collectionDao.getCollectionById(collectionId, userId)
                    if (localEntity != null) {
                        val totalCards = collectionCardDao.getTotalCardCount(localEntity.id, userId)
    ...
    override suspend fun deleteCollection(collectionId: Long): Flow<Resource<Unit>> = flow {
        ...
        try {
            ...
            val firestoreId = entity?.firestoreId?.takeIf { it.isNotEmpty() } ?: collectionId.toString()
